                    ...
                    for line in chaFile:
                        ...
                        if sndFound and phoFound:
                            ...
                            for i in range(1, n_splits):
                                ...
                                audiofilename = subject + ":" + str(startTime) + ":" + str(
                                    int(startTime) + splitDuration) + ".wav"
...
class NeuralNetwork:
    ...
    def __network(self, x):
        ...
        for n_layer in range(self.hiddenLayers):
            if n_layer == 0:
                ...
                self.biases.append(tf.Variable(tf.random_normal([self.n_hidden[n_layer]])))
                self.layer.append(
                    tf.nn.relu(tf.add(tf.matmul(self.layer[n_layer - 1], self.weights[n_layer]), self.biases[n_layer])))
        ...
        self.layer.append(tf.matmul(self.layer[self.hiddenLayers - 1], self.weights[self.hiddenLayers]) + self.biases[
            self.hiddenLayers])
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            with progressbar.ProgressBar(max_value=self.training_epochs, redirect_stdout=True,
                                         widgets=pbarWidgets) as pbar:
...
class AudioCorrection:
    def __init__(self, audiofile, tfSessionFile, segmentLength=300, segmentHop=100, n_features=80,
    ...
    def saveCorrectedAudio(self):
        ...
        outputFilenamePrefix = os.path.join(self.correctionsDir,
                                            os.path.splitext(os.path.basename(self.inputFilename))[0])
...
def run(train=False, correct=False):
    ...
    if correct:
        audiofile = 'c:\\Users\\shtosh\\test.mp3'
        if train:
